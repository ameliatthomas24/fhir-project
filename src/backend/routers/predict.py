...
# IMPORTANT: Match this to the service name in your docker-compose.yml
FHIR_URL = os.getenv("FHIR_BASE_URL", "http://hapi-fhir-server:8080/fhir")

BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
MODEL_PATH = os.path.join(BASE_DIR, "ml", "model.pkl")

# Load model with better error reporting
try:
...
async def get_prediction(patient_id: str):
    ...
    try:
        ...
        # 3. Create Profile
        # Mapping Synthea/FHIR data to your ML schema
        profile = PatientProfile(
            gender=patient.get("gender", "male").capitalize(),
            age=45.0, # Placeholder for age calculation
